"""MySQL to Snowflake cross-dialect connector."""
...
class MySQLToSnowflakeConnector(BaseBackendConnector):
    """Connector that loads data from native MySQL to Snowflake using dlt."""

    def __init__(self):
        """Initialize MySQL to Snowflake connector."""
    ...
    def connect(
        ...
        """Connect to Snowflake and load data from MySQL database using dlt."""
        ...
        if not namespace:
            raise ValueError(
                "namespace parameter is required for MySQLToSnowflake connector."
            )
        ...
        if db_path_str.startswith("minidev_mysql_"):
            db_id = db_path_str.replace("minidev_mysql_", "", 1)
        ...
        if load_data:
            ...
            print(f"Loading {len(table_names)} tables from MySQL to Snowflake...")
            ...
            pipeline = dlt.pipeline(
                pipeline_name=f"mysql_to_snowflake_{effective_schema_name}",
