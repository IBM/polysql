"""PostgreSQL to DuckDB cross-dialect connector."""
...
class PostgresToDuckDBConnector(BaseBackendConnector):
    """Connector that loads data from native PostgreSQL to DuckDB using dlt."""

    def __init__(self):
        """Initialize PostgreSQL to DuckDB connector."""
    ...
    def connect(
        ...
        """Connect to DuckDB and optionally load data from PostgreSQL source via dlt."""
    ...
    def _load_data_with_dlt(
        ...
        """Load data from PostgreSQL source to DuckDB using dlt."""
        ...
        print(f"Loading {len(table_names)} tables from PostgreSQL to DuckDB...")

        pipeline = dlt.pipeline(
            pipeline_name=f"postgres_to_duckdb_{target_db_path.stem}",
            destination=dlt.destinations.duckdb(str(target_db_path), schema="main"),
            dataset_name="main",
        )

        pg_source = _create_ibis_dlt_source(pg_conn, table_names, write_disposition)
        pipeline.run(pg_source())
        pg_conn = None
