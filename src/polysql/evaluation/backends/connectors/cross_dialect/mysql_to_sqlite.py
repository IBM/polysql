"""MySQL to SQLite cross-dialect connector."""
...
class MySQLToSQLiteConnector(BaseBackendConnector):
    """Connector that loads data from native MySQL to SQLite using dlt."""

    def __init__(self):
        """Initialize MySQL to SQLite connector."""
    ...
    def connect(
        ...
        """Connect to SQLite and optionally load data from MySQL using dlt."""
        creds = get_mysql_credentials()
        ...
        if load_data:
            mysql_conn_temp = ibis.mysql.connect(
                host=creds["host"],
                port=creds["port"],
                database=str(db_path),
                user=creds["user"],
            ...
            table_names = [
                t for t in mysql_conn_temp.list_tables() if not t.startswith("_dlt_")
            ]
            mysql_conn_temp = None
            ...
            print(f"Loading {len(table_names)} tables from MySQL to SQLite using dlt...")

            pipeline_name = f"mysql_to_sqlite_{sqlite_path.stem.split('_')[-1]}"
