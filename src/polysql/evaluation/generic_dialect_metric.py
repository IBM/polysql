...
from polysql.evaluation.execute_generic import ExecutionEngineFactory

# Query comparison timeout (60 seconds)
COMPARISON_TIMEOUT = 60


def _execute_with_timeout(func, timeout_seconds):
    ...
    def timeout_context(seconds):
        ...
        def timeout_handler(signum, frame):
            raise TimeoutError(f"Query comparison timed out after {seconds} seconds")
...
def generic_dialect_metric(
    ...
    def _run_comparison() -> MetricResult:
        ...
        try:
            if query1.query_type == "substrait":
                with ExecutionEngineFactory.create(
                    db_path,
                ...
                with ExecutionEngineFactory.create(
                    ...
                    result1 = engine.execute(query1.query)
                    query1_executed = True
                    query1_shape = result1.shape
        ...
        try:
            if query2.query_type == "substrait":
                ...
                with ExecutionEngineFactory.create(
                    ...
                    result2 = engine.execute(query2.query)
                    query2_executed = True
                    query2_shape = result2.shape
        except Exception as e:
            query2_error = str(e)
    ...
    return _execute_with_timeout(_run_comparison, COMPARISON_TIMEOUT)
